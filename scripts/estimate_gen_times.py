...
#args=argparse.Namespace(infile="/Users/cj/spaceness/sims/slimout/spatial/W50_muts/sigma_0.4494235649796564_.trees_5319950_1272000")
...
sp.run("cd ~/spaceness/; slim -d sigma="+str(sigma)+" -d id="+str(id)+" slim_recipes/gen_timer_spatial.slim",
       shell=True)
filename=re.search(os.listdir("/Users/cj/spaceness/tmp")[0],str(id))
ts = sps.SpatialSlimTreeSequence(pyslim.load("/Users/cj/spaceness/tmp/"+str(sigma)[0:6]+"_"+str(id)+".trees"), dim=2)
...
row=[sigma,np.mean(parent_ages[use_these])]

#write to file
out=open("/Users/cj/spaceness/W50sp_slimMuts_gentimes.txt","a")
