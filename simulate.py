...
simplifyTreeSequenceDirectory(indir=wd+"sims/trees/",
                              outdir=wd+"sims/sampled/",
                              nSamples=50)

#add mutations
mutateTrees(treesDirec=wd+"/sims/sampled/",
            outputDirec=wd+"/sims/mutated/",
            numMuts=1,muLow=1e-8,muHigh=1e-8)

#get summary stats from haplotype matrices (probably not working yet)
haps,positions,labels,locs=getHapsPosLabelsLocs(direc=wd+"sims/mutated/")
...
def getSumStatsFromHaplotypes(haps,positions,labels,locs,outfile,verbose=True):
    ...
    for i in range(len(haps)):
        ...
        tajD=allel.diversity.tajima_d(ac=allele_counts)
        ...
        #gen_dist=allel.pairwise_dxy(pos=positions[i],gac=genotypes) #SLOOOOOW - issue with noninteger positions?
...
out=getSumStatsFromHaplotypes(haps,dpositions,labels,locs,
                              outfile="/Users/cj/spaceness/sims/sumstats.txt")
