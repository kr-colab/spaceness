...
\reply{
ah, a confusing legend title placement! we moved "model" to after "spatial" as suggested.
}
...
\reply{
This is an excellent point also raised by other reviewers. We have added an appendix section comparing our model to a reverse-time stepping stone analysis on several relevant summary statistics \llname{appendix1}. Briefly, we find that many of the qualitative trends we see also occur in a stepping-stone setting, suggesting that they are driven purely by the particularities of our parameterization. We also see several interesting departures from the stepping-stone model reflecting shortcomings in the gridded approach to approximating continuous landscapes, which we think our model helps to avoid. We are also curious how all the other analyses we test would be affected using other simulation schemes, but hope to explore that aspect in future work. 
}
