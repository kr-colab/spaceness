...
%DIF DEL spaceness_first_revision.tex   Thu Feb 20 15:38:51 2020
%DIF ADD spaceness.tex                  Wed Feb 19 12:16:34 2020
...
%DIF -------
\newcommand{\responsefile}{second-review-responses.tex}  % name of the review reponses file %DIF > 
%DIF -------
...
%DIF 66c66-67
%DIF <         The combination of low dispersal and clustered sampling causes demographic inference from the site frequency spectrum to infer more turbulent demographic histories, but averaged results across multiple simulations were surprisingly robust to isolation by distance. 
%DIF -------
    The combination of low dispersal and clustered sampling causes demographic inference from the site frequency spectrum to infer more turbulent demographic histories, but averaged results across multiple simulations %DIF > 
    revealed surprisingly little systematic bias. %DIF > 
%DIF -------
...
\DIFdelbegin %DIFDELCMD < \revpoint{AE}{1}
%DIFDELCMD < %%%
\DIFdelend much modern work still describes geographic structure as a set of discrete populations connected by migration \citep[e.g.,][]{Wright1931,epperson2003geographical,rousset2011likelihoodbased,Shirk2014,lundgren2019populations} or as an average over such discrete models \citep{Petkova2015,al2019estimating}. 
\DIFdelbegin %DIFDELCMD < \revpoint{4}{3}
%DIFDELCMD < %%%
\DIFdelend For this reason, most population genetics statistics are interpreted with reference to discrete, well-mixed populations, and most empirical papers analyze variation within clusters of genetic variation inferred by programs like \textit{STRUCTURE} \citep{Pritchard2000} with methods that assume these are randomly mating units.
...
We simulate chromosome-scale genomic data in tens of thousands of individuals from parameter regimes relevant to common subjects of population genetic investigation\DIFdelbegin \DIFdel{such as humans and }\textit{\DIFdel{Drosophila}}%DIFAUXCMD
\DIFdelend , and output the full genealogy and recombination history of all final-generation individuals. 
...
defined \DIFaddbegin \DIFadd{in two dimensions }\DIFaddend to be $N_W = 4 \pi \rho \sigma^2$,
where \DIFdelbegin \DIFdel{$\sigma$ is the mean }\DIFdelend \DIFaddbegin \DIFadd{$\sigma^2$ is one half of the mean squared }\DIFaddend parent--offspring distance
and $\rho$ is population density
\DIFaddbegin \DIFadd{(see \mbox{%DIFAUXCMD
\citet{Rousset1997} }\hspace{0pt}%DIFAUXCMD
for further discussion of parameter definitions in one- and two-dimensional habitats) }\llabel{axialdist}\DIFaddend . 
...
\DIFdelbegin %DIFDELCMD < \revpoint{3}{2}
%DIFDELCMD < %%%
\DIFdelend This also prevents the population clumping seen by \citet{Felsenstein1975}
(Supplemental \autoref{fig:wm_v_dd})). 
\DIFdelbegin %DIFDELCMD < \revpoint{3}{1}
%DIFDELCMD < %%%
\DIFdelend Such models have been used extensively in ecological modeling
...
\DIFdelbegin %DIFDELCMD < \revpoint{AE}{2}
%DIFDELCMD < %%%
\DIFdelend where to our knowledge implementations of continuous space models
...
To reduce the \DIFdelbegin \DIFdel{parameter space }\DIFdelend \DIFaddbegin \DIFadd{number of parameters }\DIFaddend we use the same parameter, denoted $\sigma$, 
...
\DIFdelbegin \DIFdel{As in previous work \mbox{%DIFAUXCMD
\citep{Wright1943,Ringbauer2017}}\hspace{0pt}%DIFAUXCMD
, }\DIFdelend \DIFaddbegin \DIFadd{Informally, we think of }\DIFaddend $\sigma$ \DIFdelbegin \DIFdel{is equal to the mean parent-offspring distance}\DIFdelend \DIFaddbegin \DIFadd{as the ``dispersal distance'',
although only one of those stages is dispersal}\DIFaddend .
...
\DIFdelbegin %DIFDELCMD < \revpoint{4}{8}
%DIFDELCMD < %%%
\DIFdelend Each offspring is produced with a mate selected randomly from those within distance $3 \sigma$,
with probability of choosing a neighbor at distance $d$ proportional to \DIFdelbegin \DIFdel{$\exp(-d^2 / 2 \sigma^2)$}\DIFdelend \DIFaddbegin \DIFadd{the Gaussian density
with mean zero and standard deviation $\sigma$, which is $g(d) = \exp(-d^2 / 2 \sigma^2)/ (2 \sigma^2)$}\DIFaddend .

To maintain a stable population, mortality increases with local population density.
To do this we say that individuals at distance $d$ have a competitive interaction with strength $g(d)$\DIFdelbegin \DIFdel{,
where $g$ is the Gaussian density with mean zero and standard deviation $\sigma$}\DIFdelend .
...
\DIFdelbegin %DIFDELCMD < \revpoint{3}{6}
%DIFDELCMD < %%%
\DIFdelend we also implemented simulations identical to those above except that mates are selected uniformly at random from the population,
...
\DIFdelbegin %DIFDELCMD < \revpoint{AE}{3}
%DIFDELCMD < %%%
\DIFdelend with per-unit carrying capacity $K=5$ (census $N \approx 10,000$), 
...
\DIFdelbegin %DIFDELCMD < \revpoint{2}{5}
%DIFDELCMD < %%%
\DIFdelend We verify that this procedure produces the same site frequency spectrum as applying mutations directly in SLiM in Figure \ref{fig:slim_v_msp_sfs},
...
\DIFdelbegin %DIFDELCMD < \revpoint{1}{2}
%DIFDELCMD < %%%
\DIFdelend mean life span ($L$), and dispersal distance ($\sigma$).
However, nonlinearity of local demographic stochasticity causes
actual realized averages of these demographic \DIFdelbegin \DIFdel{quantitites }\DIFdelend \DIFaddbegin \DIFadd{quantities }\DIFaddend to deviate from the specified values
...
to compute neighborhood size as $N_W = 4 \pi \rho \sigma^2$.  \DIFdelbegin %DIFDELCMD < \revpoint{3}{6}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \llabel{ll:nw_defn}
...
using pyslim \citep{pyslim}.
 ...
 \DIFaddbegin \DIFadd{Note that $\sigma$ controls the dispersal of offspring away from only }\emph{\DIFadd{one}} \DIFadd{parent }\llabel{ll:sigma_effective}
\DIFadd{(e.g., the seed parent for plants),
while population genetics usually defines "dispersal distance" to be the distance to a randomly chosen one of the two parents
...
To match theory, neighborhood size should be defined in terms of }\emph{\DIFadd{effective}}
\DIFadd{dispersal distance, i.e., the mean squared displacement along an axis
between parent-child pairs found moving back along a lineage \mbox{%DIFAUXCMD
\citep{barton2002neutral}}\hspace{0pt}%DIFAUXCMD
, 
and }\emph{\DIFadd{effective}} \DIFadd{population density \mbox{%DIFAUXCMD
\citep{Rousset1997}}\hspace{0pt}%DIFAUXCMD
.
However, we use $\sigma$ and $\rho$ as defined here to compute $N_W$
...
\DIFdelbegin %DIFDELCMD < \revpoint{1}{1}
%DIFDELCMD < %%%
\DIFdelend The full distribution of lengths of IBS tracts for each pair of chromosomes was first calculated with a custom python function. 
...
The effects of sampling on summary statistic estimates were summarized by testing for differences in mean (ANOVA, \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Rcore2018}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{Rcore2018}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend ) and variance (Levene's test, \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Fox2011}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{Fox2011}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend ) across sampling strategies for each summary statistic. 


%%%%%%%%%%%
\subsection{Demographic Inference}
\DIFdelbegin %DIFDELCMD < \revpoint{1}{4}
%DIFDELCMD < %%%
\DIFdelend 
...
\DIFdelbegin %DIFDELCMD < \revpoint{1}{5}
%DIFDELCMD < %%%
\DIFdelend and a mean that may depend on spatial position.
...
\DIFdelbegin %DIFDELCMD < \revpoint{4}{10}
%DIFDELCMD < %%%
\DIFdelend Third, we simulated a more concentrated ``\emph{corner}'' environmental effect by setting the mean phenotype to 120
...
\DIFdelbegin %DIFDELCMD < \revpoint{1}{6}
%DIFDELCMD < %%%
\DIFdelend by comparing to the values expected from a uniform distribution
...
\DIFdelbegin %DIFDELCMD < \revpoint{1}{11}
%DIFDELCMD < %%%
\DIFdelend were the same in all simulations,
...
\DIFdelbegin %DIFDELCMD < \revpoint{1}{9}
%DIFDELCMD < %%%
\DIFdelend 
...
Essentially, PCA seems unable to effectively summarize the weak population structure present in large-neighborhood simulations \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{given the sample sizes we tested, }\revpoint{2}{2}
...
Previous studies have found that population structure and nonrandom sampling can create spurious signals of population bottlenecks when attempting to infer demographic history with microsatellite variation, summary statistics, or runs of homozygosity \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Chikhi2010,Stadler2009,Ptak2002,Mazet2015}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Chikhi2010,Stadler2009,Ptak2002,Mazet2015,leblois2006genetics}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . Here we found that methods that infer detailed population trajectories through time based on the SFS and patterns of LD across the genome are also subject to this bias, with some combinations of dispersal and sampling strategy systematically inferring deep recent population bottlenecks and overestimating ancient $N_{e}$ by around a factor of 2. We were surprised to see that both stairwayplot and SMC++ can tolerate relatively strong isolation by distance -- i.e., neighborhood sizes of 20 -- and still perform well when averaging results across multiple simulations.
\DIFaddbegin \DIFadd{(However, note the high amount of between-simulation variance seen in }\autoref{fig:demography_supplement}\DIFadd{.) }\llabel{ll:variance_note}
\DIFaddend Inference in populations with neighborhood sizes over 20 was relatively unbiased unless samples were concentrated in the middle of the range (Figure \ref{fig:demography}). 
...
\citep{Khera2018} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{wray2007prediction,international2009common} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend are likely to be affected by subtle population structure. \DIFaddbegin \revpoint{2}{3}
\DIFaddend Indeed as recently identified in studies of genotype associations for human height in Europe \citep{Berg2018,Sohail2018}, \DIFaddbegin \revpoint{2}{4}
\DIFaddend PC regression GWAS in modern human populations do include residual signal of population structure in large-scale analyses of polygenic traits. \DIFdelbegin \DIFdel{When attempting to make predictions across populations with different environmental exposures, polygenic risk scores affected }\DIFdelend \DIFaddbegin \DIFadd{In addition to error associated with varying patterns of linkage disequilibrium and allele frequency among populations, the confounding of environmental and genetic effects on phenotypes introduced }\DIFaddend by population structure \DIFdelbegin \DIFdel{can be expected to offer }\DIFdelend \DIFaddbegin \DIFadd{is expected to lead to }\DIFaddend low predictive power \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{when polygenic scores are generated for populations outside the original GWAS cohort, }\llabel{ldcaveats}
%DIF > When attempting to make predictions across populations with different environmental exposures, polygenic risk scores affected by population structure can be expected to offer low predictive power, 
\DIFaddend as was shown in a recent study finding lower \DIFdelbegin \DIFdel{performance }\DIFdelend \DIFaddbegin \DIFadd{polygenic score predictive power }\DIFaddend outside European populations \citep{Martin2019}.
...
\DIFdelbegin \DIFdel{is space 
}\DIFdelend \DIFaddbegin \DIFadd{are associated with geography }\DIFaddend (note this is likely not the case for some modern human populations): 
...
\DIFdelbegin %DIFDELCMD < \revpoint{2}{8}
%DIFDELCMD < %%%
...
\begin{tabular}{rllrrll}
 ...
 Species & Description & \makecell[l]{Neighborhood\\Size} & Method & Citation \\ 
  ...
  \DIFdelbeginFL %DIFDELCMD < \makecell[l]{\textit{Bebicium}\\\textit{vittatum}} & %%%
\DIFdelFL{intertidal snail }%DIFDELCMD < & %%%
\DIFdelFL{240,000 }%DIFDELCMD < & %%%
\DIFdelFL{Survey }%DIFDELCMD < & %%%
\DIFdelFL{\mbox{%DIFAUXCMD
\citep{Rousset1997}}\hspace{0pt}%DIFAUXCMD
}%DIFDELCMD < \\ 
%DIFDELCMD <   \makecell[l]{\textit{Bebicium}\\\textit{vittatum}} & %%%
\DIFdelFL{intertidal snail }%DIFDELCMD < & %%%
\DIFdelFL{360,000 }%DIFDELCMD < & %%%
\DIFdelFL{Genetic }%DIFDELCMD < & %%%
\DIFdelFL{\mbox{%DIFAUXCMD
\citep{Rousset1997}}\hspace{0pt}%DIFAUXCMD
}%DIFDELCMD < \\ 
%DIFDELCMD <    %%%
\DIFdelendFL %DIF > \makecell[l]{\textit{Bebicium}\\\textit{vittatum}} & intertidal snail & 240,000 & Survey & \citep{Rousset1997}\\ 
  %DIF > \makecell[l]{\textit{Bebicium}\\\textit{vittatum}} & intertidal snail & 360,000 & Genetic & \citep{Rousset1997}\\ 
   \hline
\end{tabular}
\label{table:NStable}
\end{adjustwidth}
...
\DIFdelbegin \DIFdel{are more strongly determined }\DIFdelend by global $N_e$\DIFdelbegin \DIFdel{or by }\DIFdelend \DIFaddbegin \DIFadd{, or }\DIFaddend the ratio of the two. 
...
In addition, \DIFaddbegin \DIFadd{definitions of "population density" in genetic versus ecological studies may lead to varying estimates of neighborhood size for a given species }\llabel{rhocaveat}\DIFadd{, and }\DIFaddend these empirical examples \DIFdelbegin \DIFdel{are likely }\DIFdelend \DIFaddbegin \DIFadd{may be }\DIFaddend biased towards small-neighborhood species \DIFdelbegin \DIFdel{(}\DIFdelend because few studies have quantified neighborhood size in species with very high dispersal or population density\DIFdelbegin \DIFdel{). 
}\DIFdelend \DIFaddbegin \DIFadd{. 
}

\DIFaddend However, from the available data we find that neighborhood sizes in the range we simulated are fairly common across a range of taxa. At the extreme low end of empirical neighborhood size estimates we see some flowering plants, large mammals, and colonial insects like ants \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{with neighborhood sizes less than roughly 100. }\DIFaddend Species such as this have neighborhood size estimates small enough that spatial processes are likely to strongly influence inference.
...
\DIFdelbegin %DIFDELCMD < \revpoint{3}{7}
%DIFDELCMD < %%%
...
\DIFdelbegin \DIFdel{Since populations are still }\DIFdelend \DIFaddbegin \DIFadd{As is usual in population genetics, }\revpoint{2}{6}
\DIFadd{the populations are }\DIFaddend entirely \emph{intrinsically} regulated\DIFdelbegin \DIFdel{,
our model still has a very strong ``population genetics'' flavor}\DIFdelend .
...
\DIFaddend Much additional work remains to be done to better understand how \DIFdelbegin \DIFdel{these parameters }\DIFdelend \DIFaddbegin \DIFadd{life history, range size,
and habitat geometry }\DIFaddend interact to shape genetic variation in continuous space, which we leave to future studies. 
...
we posit that process-driven descriptions of ancestry  \DIFaddbegin \revpoint{2}{7}
\DIFaddend and/or more generalized unsupervised methods may be able to better account for carry out this task.
...
%DIF <  We thank the Hearth for having such good, nearby coffee, and
%DIF <  Falling Sky Brewing for creative support during manuscript drafting.
\DIFaddbegin \DIFadd{Three anonymous reviewers and the suggestions from editors of GENETICS significantly improved the final manuscript. 
}\DIFaddend CJB and ADK were supported by NIH award R01GM117241. 
\DIFdelbegin %DIFDELCMD < \revpoint{AE}{4}
%DIFDELCMD < %%%
\DIFdelend 
...
For example, $\theta_\pi$ is \DIFdelbegin \DIFdel{inflated }\DIFdelend \DIFaddbegin \DIFadd{lower in stepping-stone models 
}\DIFaddend at low neighborhood sizes (i.e., low $m$),
\DIFdelbegin \DIFdel{and the extent of the inflation }\DIFdelend \DIFaddbegin \DIFadd{but }\DIFaddend increases to approach the continuous case as the resolution of the landscape increases.
Similar patterns are observed for \DIFdelbegin \DIFdel{$F_{IS}$ and }\DIFdelend observed heterozygosity. However, $\theta_{W}$ behaves differently, showing a non-monotonic relationship with grid resolution.
This results in an increasingly positive Tajima's $D$ in grid simulations at small neighborhood sizes, to a much greater extent than seen in a continuous model. In contrast to $\theta_\pi$, increasing the resolution of the grid causes Tajima's $D$ to deviate \emph{more} from what is seen in the continuous model\DIFaddbegin \DIFadd{. 
...
at intermediate neighborhood sizes the continuous case best matches intermediate grid resolutions}\DIFaddend .
