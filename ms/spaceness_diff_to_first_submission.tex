...
%DIF DEL spaceness-oldtmp-2802.tex   Fri Dec  6 09:35:39 2019
%DIF ADD spaceness.tex               Fri Dec  6 09:35:37 2019
%DIF 2-15c2
...
%DIF <      }
%DIF < 
%DIF < \documentclass[11pt,twoside,lineno,hidelinks]{preprint}
%DIF -------
\documentclass[10pt,twoside,lineno,hidelinks]{preprint} %DIF > 
%DIF -------
\usepackage{amsmath}
...
%DIF 34a22-29
%%%%%%%%%%% %DIF > 
% for responses to reviewers %DIF > 
% set this to show line numbers and include responses to reviews or not %DIF > 
\newif\ifreviewresponses %DIF > 
\reviewresponsestrue  % include them %DIF > 
% \reviewresponsesfalse  % don't include them %DIF > 
\input{review-response-commands} %DIF > 
\newcommand{\responsefile}{review-responses.tex}  % name of the review reponses file %DIF > 
%DIF -------
...
%DIF 67a65
    Stepping-stone models reproduce some of these effects, but discretizing the landscape introduces artifacts which in some cases are exacerbated at higher resolutions. %DIF > 
%DIF -------
...
\citet{Wright1943,Rousset1997,barton2002neutral,Barton2010,Ringbauer2017,robledoarnuncio2010isolation}}\hspace{0pt}%DIFAUXCMD
), }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{Wright1943,Rousset1997,barton2002neutral,Barton2010,Ringbauer2017,robledoarnuncio2010isolation,Wilkins2002linearcoalescent,Wilkins2004separation}}\hspace{0pt}%DIFAUXCMD
), }\revpoint{AE}{1}
\DIFaddend much modern work still describes geographic structure as a set of discrete populations connected by migration \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep[e.g.,][]{Wright1931,epperson2003geographical,rousset2011likelihoodbased,Shirk2014,lundgren2018populations}}\hspace{0pt}%DIFAUXCMD
. }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep[e.g.,][]{Wright1931,epperson2003geographical,rousset2011likelihoodbased,Shirk2014,lundgren2019populations} }\hspace{0pt}%DIFAUXCMD
or as an average over such discrete models \mbox{%DIFAUXCMD
\citep{Petkova2015,al2019estimating}}\hspace{0pt}%DIFAUXCMD
. }\revpoint{4}{3}
\DIFaddend For this reason, most population genetics statistics are interpreted with reference to discrete, well-mixed populations, and most empirical papers analyze variation within clusters of genetic variation inferred by programs like \textit{STRUCTURE} \citep{Pritchard2000} with methods that assume these are randomly mating units.
...
A key early observation by \DIFdelbegin \DIFdel{Wright \mbox{%DIFAUXCMD
\citep{Wright1946} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{Wright1946} }\hspace{0pt}%DIFAUXCMD
...
\citep{epperson2003geographical}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , 
...
To avoid \DIFdelbegin \DIFdel{questionable assumptions}\DIFdelend \DIFaddbegin \DIFadd{hard-to-evaluate approximations}\DIFaddend , we here used forward-time, individual-based simulations
\DIFdelbegin \DIFdel{. By scaling the }\DIFdelend \DIFaddbegin \DIFadd{across continuous geographical space.
The question of what regulates real populations has a long history and many answers
\mbox{%DIFAUXCMD
\citep[e.g.,][]{lloyd1967crowding,antonovics1980ecological,crawley1990population}}\hspace{0pt}%DIFAUXCMD
, }\llabel{ecolcites}
\DIFadd{but it is clear that populations must at some point have density-dependent feedback on population size,
...
and there are many ways to achieve this \mbox{%DIFAUXCMD
\citep{bolker2003spatial}}\hspace{0pt}%DIFAUXCMD
.
In our simulations,
each individual's }\DIFaddend probability of survival \DIFdelbegin \DIFdel{in each timestep to }\DIFdelend \DIFaddbegin \DIFadd{is a decreasing function of }\DIFaddend local population density\DIFdelbegin \DIFdel{we shift }\DIFdelend \DIFaddbegin \DIFadd{,
which shifts }\DIFaddend reproductive output towards low-density regions,
\DIFdelbegin \DIFdel{which prevents populations from clustering. }\DIFdelend \DIFaddbegin \DIFadd{and produces total census sizes that fluctuate around an equilibrium. }\revpoint{3}{2}
\DIFadd{This also prevents the population clumping seen by \mbox{%DIFAUXCMD
\citet{Felsenstein1975}
}\hspace{0pt}%DIFAUXCMD
(Supplemental }\autoref{fig:wm_v_dd}\DIFadd{)). }\revpoint{3}{1}
\DIFaddend Such models have been used extensively in ecological modeling
\DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{durrett1994importance,bolker1997using,law2003population,fournier2004microscopic,champer2019suppression}
}\hspace{0pt}%DIFAUXCMD
}\DIFaddend but rarely in population genetics, \DIFaddbegin \revpoint{AE}{2}
\DIFaddend where to our knowledge \DIFdelbegin \DIFdel{previous }\DIFdelend implementations of continuous space models
\DIFaddbegin \DIFadd{before their availability through SLiM \mbox{%DIFAUXCMD
\citep{Haller2019}
}\hspace{0pt}%DIFAUXCMD
}\DIFaddend have focused on a small number of genetic loci
\DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep[e.g.,][]{slatkin1989comparison,barton2002neutral,robledoarnuncio2010isolation,rossine2014espao,jackson2014landscape}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend ,
...
We simulated populations using the \DIFdelbegin \DIFdel{non-Wright-Fisher module in the }\DIFdelend program SLiM v3.1 \citep{Haller2019}. 
...
Individuals are hermaphroditic, and each time step, each produces a Poisson number of offspring with mean $1/L$\DIFdelbegin \DIFdel{where $L$ is the expected lifespan}\DIFdelend .
Offspring disperse a Gaussian-distributed distance away from the parent with mean zero and standard deviation $\sigma$
in both the $x$ and $y$ coordinates. \DIFaddbegin \revpoint{4}{8}
\DIFaddend Each offspring is produced with a mate selected randomly from those within distance $3 \sigma$,
with probability of choosing a neighbor at distance \DIFdelbegin \DIFdel{$x$ proportional to $\exp(-x^2 / 2 \sigma^2)$}\DIFdelend \DIFaddbegin \DIFadd{$d$ proportional to $\exp(-d^2 / 2 \sigma^2)$}\DIFaddend .
...
Then, given a per-unit carrying capacity $K$, \DIFaddbegin \llabel{ll:K_defn}
\DIFaddend the probability of survival until the next time step for individual $i$ is
\begin{equation} \DIFaddbegin \label{eqn:p_survival}
    \DIFaddend p_i 
    =
    \min\left( 0.95, \frac{1}{1 + n_i / (K (1+L))} \right) .
\end{equation}
We chose this functional form so that the equilibrium population density per unit area is \DIFdelbegin \DIFdel{around }\DIFdelend \DIFaddbegin \DIFadd{close to }\DIFaddend $K$, \DIFaddbegin \llabel{ll:K_again}
\DIFaddend and the mean lifetime is around $L$\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{;
for more description see the Appendix. %DIF >  \ref{apx:demographic_model}.
}\DIFaddend 
...
This buffer roughly counteracts the increase in fitness individuals close to the edge would otherwise have\DIFaddbegin \DIFadd{, though the effect is relatively subtle (Figure \ref{fig:edge_scaling})}\DIFaddend .

To isolate spatial effects from other components of the model such as overlapping generations, increased variance in reproductive success, and density-dependent fitness,  \DIFaddbegin \revpoint{3}{6}
\DIFaddend we also implemented simulations identical to those above except that mates are selected uniformly at random from the population,
...
Scripts for figures and analyses are available at \DIFdelbegin %DIFDELCMD < \url{https://github.com/petrelharp/spaceness}%%%
\DIFdelend \DIFaddbegin \url{https://github.com/kern-lab/spaceness}\DIFaddend .  

We ran 400 simulations for the spatial and random-mating models on a square landscape of width $W=50$  \DIFaddbegin \revpoint{AE}{3}
\DIFaddend with per-unit carrying capacity $K=5$ (census $N \approx 10,000$), 
average lifetime $L=4$, 
genome size \DIFdelbegin \DIFdel{= }\DIFdelend $10^{8}$\DIFaddbegin \DIFadd{bp}\DIFaddend , 
recombination rate \DIFdelbegin \DIFdel{= }\DIFdelend $10^{-9}$ \DIFaddbegin \DIFadd{per bp per generation}\DIFaddend , 
...
by the average generation time estimated for each value of $\sigma$ (see `Demographic Parameters' below) to convert the rate to units of mutations per site per unit time. \DIFdelbegin \DIFdel{(We verified }\DIFdelend \DIFaddbegin \revpoint{2}{5}
\DIFadd{We verify }\DIFaddend that this procedure \DIFdelbegin \DIFdel{produced the correct number of mutations by comparing to a subset of simulations with SLiM-generated mutations,
which are applied only at meiosis.
)
}\DIFdelend \DIFaddbegin \DIFadd{produces the same site frequency spectrum as applying mutations directly in SLiM in Figure \ref{fig:slim_v_msp_sfs},
in agreement with theory \mbox{%DIFAUXCMD
\citep{ralph2019efficiently}}\hspace{0pt}%DIFAUXCMD
.
}\DIFaddend Simulations were run for 1.6 million timesteps (approximately $30N$ generations). 

\DIFaddbegin \DIFadd{We also compared our model's output to a commonly-used approximation of continuous space,
the stepping-stone model, which we simulated with }\texttt{\DIFadd{msprime}} \DIFadd{\mbox{%DIFAUXCMD
\citep{Kelleher2016}}\hspace{0pt}%DIFAUXCMD
.
...
which differed from the continuous model }\emph{\DIFadd{more}} \DIFadd{at finer discretizations.
}

\DIFaddend %%%%%%%%%%%%%
\subsection{Demographic Parameters}

Our demographic model includes parameters \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{that control }\DIFaddend population density ($K$), \DIFaddbegin \revpoint{1}{2}
\DIFaddend mean life span ($L$), and dispersal distance ($\sigma$).
...
We recorded the census population size in all simulations\DIFdelbegin \DIFdel{. 
}\DIFdelend \DIFaddbegin \DIFadd{,
and used mean population density ($\rho$, census size divided by total area)
to compute neighborhood size as $N_W = 4 \pi \rho \sigma^2$. }\revpoint{3}{6}
\DIFaddend To estimate generation times, we stored ages of the parents of every new individual born across 200 timesteps, 
after a 100 generation burn-in, and took the mean. 
To estimate variance in offspring number, 
we tracked the \DIFaddbegin \DIFadd{lifetime total }\DIFaddend number of offspring for all individuals for 100 timesteps following a 100-timestep burn-in period,
\DIFdelbegin \DIFdel{subset the resulting table to include only the last timestep recorded for each individual, }\DIFdelend %DIF >  subset the resulting table to include only the last timestep recorded for each individual,
...
using pyslim \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citetext{\url{https://github.com/tskit-dev/pyslim}}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{pyslim}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
as well as \DIFdelbegin \DIFdel{an isolation-by-distance regression of genetic distance ($D_{xy}$) against the logarithm of geographic distance analogous to \mbox{%DIFAUXCMD
\cite{Rousset1997}}\hspace{0pt}%DIFAUXCMD
's approach, 
which we summarized as }\DIFdelend \DIFaddbegin \DIFadd{(motivated by \mbox{%DIFAUXCMD
\citet{Rousset1997}}\hspace{0pt}%DIFAUXCMD
's results)
}\DIFaddend the correlation coefficient between the logarithm of the spatial distance and the proportion of identical base pairs across pairs of individuals. 
%DIF >  as an isolation-by-distance regression of genetic distance ($D_{xy}$) against the logarithm of geographic distance analogous to \citet{Rousset1997}'s approach, which we summarized as 

Following recent studies that showed strong signals for dispersal and demography in the distribution of shared haplotype block lengths \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Ringbauer2017,Baharian2016}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep[e.g.,][]{Ringbauer2017,Baharian2016}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , we also calculated various summaries of the distribution of pairwise identical-by-state (IBS) block lengths among sampled chromosomes\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{,
defined to be the set of distances between adjacent sites that differ between the two chromosomes. }\revpoint{1}{1}
\DIFaddend The full distribution of lengths of IBS tracts for each pair of chromosomes was first calculated with a custom python function. 
%DIF < \url{https://github.com/petrelharp/spaceness/blob/master/scripts/slimtools.py}. 
%DIF > \url{https://github.com/cjbattey/spaceness/blob/master/scripts/slimtools.py}. 
...
%DIFDELCMD < %%%
\DIFdel{We then estimated }\DIFdelend \DIFaddbegin \DIFadd{We then calculated }\DIFaddend correlation coefficients between spatial distance and each moment of the pairwise IBS tract distribution. Because more closely related individuals on average share longer haplotype blocks we expect that spatial distance will be negatively correlated with mean haplotype block length, and that this correlation will be strongest (i.e., most negative) when dispersal is low. The variance, skew, and count of long haplotype block statistics are meant to reflect the relative length of the right (upper) tail of the distribution, which represents the frequency of long haplotype blocks, and so should reflect recent demographic events \citep{chapman2002effect}. For a subset of simulations, we also calculated cumulative distributions for IBS tract lengths across pairs of distant (\DIFdelbegin \DIFdel{${}>48$ }\DIFdelend \DIFaddbegin \DIFadd{more than 40 }\DIFaddend map units) and nearby (\DIFdelbegin \DIFdel{${}<2$ }\DIFdelend \DIFaddbegin \DIFadd{less than 10 }\DIFaddend map units) individuals. Last, we examined the relationship between allele frequency and the spatial dispersion of an allele by calculating the average distance \DIFdelbegin \DIFdel{among }\DIFdelend \DIFaddbegin \DIFadd{between }\DIFaddend individuals carrying each derived allele\DIFdelbegin \DIFdel{in a set of simulations representing a range of neighborhood sizes}\DIFdelend .
...
%%%%%%%%%%%
\subsection{Demographic \DIFdelbegin \DIFdel{Modeling}\DIFdelend \DIFaddbegin \DIFadd{Inference}\DIFaddend }
\DIFaddbegin \revpoint{1}{4}
\DIFaddend 
...
is determined by \DIFdelbegin \DIFdel{adding independent Gaussian noise with mean zero and }\DIFdelend \DIFaddbegin \DIFadd{drawing from a Gaussian distribution with }\DIFaddend standard deviation 10 \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \revpoint{1}{5}
\DIFadd{and }\DIFaddend a mean that may depend on spatial position.
\DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{In spatially varying models, the mean phenotype differs by two standard deviations across the landscape.
We then }\DIFaddend adjust the geographic pattern of mean phenotype to create \DIFaddbegin \DIFadd{four types of }\DIFaddend spatially autocorrelated environmental influences on phenotype.
...
Concretely, \DIFaddbegin \DIFadd{the mean phenotype $p$ for }\DIFaddend an individual at position $(x,y)$ \DIFdelbegin \DIFdel{in a $50 \times 50$ landscape has mean phenotype $100 + 2x / 5$. }\DIFdelend \DIFaddbegin \DIFadd{is $p=100 + 2x / 5$. }\revpoint{4}{10}
\DIFaddend Third, we simulated a more concentrated ``\emph{corner}'' environmental effect by setting the mean phenotype \DIFaddbegin \DIFadd{to 120
}\DIFaddend for individuals with both $x$ and $y$ coordinates below 20 \DIFdelbegin \DIFdel{to 120 }\DIFdelend (two standard deviations above the rest of the map). 
...
We also examined $p$ values for \DIFdelbegin \DIFdel{systemic inflation by estimating the expected values }\DIFdelend \DIFaddbegin \DIFadd{systematic inflation }\revpoint{1}{6}
\DIFadd{by comparing to the values expected }\DIFaddend from a uniform distribution
(because no SNPs were used when generating phenotypes\DIFdelbegin \DIFdel{), plotting observed against expected values for all simulations, and summarizing across simulations by finding the mean $\sigma$ value in each region of quantile-quantile space.
}\DIFdelend \DIFaddbegin \DIFadd{, well-calibrated $p$-values should be uniform).
}

\DIFaddend Results from all analyses were summarized and plotted with the ``ggplot2'' \citep{Wickham2016} and ``cowplot'' \citep{Wilke2019} packages in R \citep{Rcore2018}. 
...
%%%%%%%%%%%
\subsection{Demographic Parameters \DIFaddbegin \DIFadd{and Run Times}\DIFaddend }
...
the generation time, census population size, and variance in offspring number\DIFdelbegin \DIFdel{.
These are }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend shown in Figure~\ref{fig:genparams}. \DIFdelbegin \DIFdel{This }\DIFdelend \DIFaddbegin \DIFadd{Because our simulation is paramaterized on an individual level, these population parameters emerge as a property of the interactions among individuals rather than being directly set. }\llabel{emergentparams}
\DIFadd{Variation across runs }\DIFaddend occurs because,
even \DIFdelbegin \DIFdel{through the ``population density '' (}\DIFdelend \DIFaddbegin \DIFadd{though the parameters }\DIFaddend $K$ \DIFdelbegin \DIFdel{) and ``mean lifetime '' (}\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend $L$ \DIFdelbegin \DIFdel{) parameters
}\DIFdelend \DIFaddbegin \DIFadd{that control population density and mean lifetime respectively }\llabel{ll:K_more} \revpoint{1}{11}
\DIFaddend were the same in all simulations,
the strength of stochastic effects depends strongly on \DIFaddbegin \DIFadd{the spatial interaction distance }\DIFaddend $\sigma$.
For instance, 
...
\DIFdelbegin \DIFdel{By a neighborhood sizes larger than 100, the spatial }\DIFdelend \DIFaddbegin \DIFadd{Spatial }\DIFaddend and random mating models are indistinguishable from one another
\DIFdelbegin \DIFdel{, 
a sign that our simulations are performing as expected. 
}\DIFdelend \DIFaddbegin \DIFadd{for neighborhood sizes larger than 100.
}\DIFaddend Census sizes range from \DIFdelbegin \DIFdel{$\approx 14,000$ }\DIFdelend \DIFaddbegin \DIFadd{around 14,000 }\DIFaddend at low $\sigma$ in the random mating model to \DIFdelbegin \DIFdel{$\approx 10,000$ }\DIFdelend \DIFaddbegin \DIFadd{10,000 }\DIFaddend for both models when neighborhood sizes approach 1,000.
\DIFaddbegin \DIFadd{The scaling of census sizes in both random-mating and spatial models appears to be related to two consequences of the spatial competition function: the decline of fitness at range edges, which effectively reduces the habitable area by one $\sigma$ around the edge of the map and so results in a smaller habitable area at high $\sigma$ values; and variation in the equilibrium population density given varying competition radii.
Furthermore, census size increases in spatial models as neighborhood size increases from 2 to 10.
This may reflect an Allee effect \mbox{%DIFAUXCMD
\citep{Allee1949} }\hspace{0pt}%DIFAUXCMD
in which some individuals are unable to find mates when the mate selection radius is very small.  }\llabel{censusscaling}
\DIFaddend 
...
\caption{\DIFaddFL{Run times of continuous space simulations with landscape width 50 and expected density 5 under varying neighborhood size. Times are shown for simulations run with mutations applied directly in SLiM (dashed lines) or later applied to tree sequences with msprime (solid lines). Times for simulations run with tree sequence recording disabled are shown in grey.}}
\label{fig:runtimes}
\end{figure}

\DIFaddend Generation time similarly shows complex behavior with respect to neighborhood sizes, and varies between 5.2 and 4.9 timesteps per generation across the parameter range explored. 
...
Even though certain aspects of population demography depend on the scale of spatial interactions, it still could be that population genetic variation is well-described by a well-mixed population model. Indeed, mathematical results suggest that genetic variation in some spatial models should be well-approximated by a Wright-Fisher population if neighborhood size is large and all samples are geographically widely separated \citep{wilkins2004separation,zahle2005stepping}. However, the behavior of most common population genetic summary statistics other than Tajima's $D$ \citep{Stadler2009} has not yet been described in realistic geographic models. Moreover, as we will show, spatial sampling strategies can affect summaries of \DIFaddbegin \DIFadd{genetic }\DIFaddend variation at least as strongly as the underlying population dynamics. \DIFaddbegin \revpoint{1}{9}
\DIFaddend 


\begin{figure*}[p]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figures/sfs_w_sumstats.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figures/sfs_w_sumstats_loglog.pdf}
\DIFaddendFL \caption{Site frequency spectrum (A\DIFaddbeginFL \DIFaddFL{; note axes are log-scaled}\DIFaddendFL ) and summary statistic distributions (B) by sampling strategy and neighborhood size.}
\label{fig:sumstats}
\end{figure*}
...
Figure \ref{fig:sumstats} shows the effect of varying neighborhood size and sampling strategy on the site frequency spectrum (Figure~\ref{fig:sumstats}\DIFdelbegin \DIFdel{A}\DIFdelend \DIFaddbegin \DIFadd{, Figure \ref{fig:allsfs}}\DIFaddend ) and several standard population genetic summary statistics (Figure~\ref{fig:sumstats}B\DIFaddbegin \DIFadd{; additional statistics are shown in Figure \ref{fig:allsumstats}}\DIFaddend ). Consistent with findings in island and stepping stone simulations \citep{Stadler2009}, the SFS shows a significant enrichment of intermediate frequency variants in comparison to the nonspatial expectation. This bias is most pronounced below \DIFdelbegin \DIFdel{neighborhood sizes $\leq 100$ }\DIFdelend \DIFaddbegin \DIFadd{a neighborhood size of 100 }\DIFaddend and is exacerbated by midpoint and point sampling of individuals (depicted in Figure~\ref{fig:samplemap}). 
...
\begin{figure*}[p]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.5\textwidth]{figures/ibs_dists2.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.5\textwidth]{figures/ibs_cumdists_w_rm.pdf}
\DIFaddendFL \caption{
    Cumulative distributions for IBS tract lengths per pair of individuals at different geographic distances, across three neighborhood sizes (\DIFdelbeginFL \DIFdelFL{$N_{W}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{NS}\DIFaddendFL ).
    \DIFaddbeginFL \DIFaddFL{Nearby pairs (red curves) share many more long IBS tracts than do distant pairs (blue curves),
...
Figure \ref{fig:demography}A shows \DIFdelbegin \DIFdel{the median }\DIFdelend \DIFaddbegin \DIFadd{rolling medians of }\DIFaddend inferred population size histories from each method across all simulations, grouped by neighborhood size and sampling strategy.
In general these methods tend to slightly overestimate ancient population sizes and infer recent population declines when neighborhood sizes are below 20 and sampling is spatially clustered\DIFdelbegin \DIFdel{(Figure \ref{fig:demography}A, Figure \ref{fig:demography_supplement})}\DIFdelend .
...
However, individual \DIFdelbegin \DIFdel{model fits from both methods frequently reflected }\DIFdelend \DIFaddbegin \DIFadd{simulations were often inferred to have }\DIFaddend turbulent demographic histories\DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{,
as shown by the individually inferred histories (shown in }\DIFaddend Figure \ref{fig:demography_supplement})\DIFdelbegin \DIFdel{, with }\DIFdelend \DIFaddbegin \DIFadd{.
Indeed, }\DIFaddend the standard deviation of inferred $N_{e}$ across time points \DIFdelbegin \DIFdel{often exceeding }\DIFdelend \DIFaddbegin \DIFadd{(shown in Figure \ref{fig:demography}B) often exceeds }\DIFaddend the expected $N_{e}$ for both methods\DIFdelbegin \DIFdel{(Figure \ref{fig:demography}B)}\DIFdelend .
That is, despite the \DIFaddbegin \DIFadd{nearly }\DIFaddend constant population sizes in our simulations, both methods tended to infer large fluctuations in population size over time, which could potentially result in incorrect biological interpretations. On average the variance of inferred population sizes was elevated at the lowest neighborhood sizes and declines as dispersal increases, with the strongest effects seen in stairwayplot \DIFdelbegin \DIFdel{model fits with for }\DIFdelend \DIFaddbegin \DIFadd{results with }\DIFaddend clustered sampling and neighborhood sizes less than 20 (Figure \ref{fig:demography}B). 
...
Figure \ref{fig:gwas}D shows quantile-quantile plots \DIFaddbegin \DIFadd{for a subset of simulations }\DIFaddend that show the degree of genome-wide inflation of test statistics in PC-corrected GWAS across all simulations and environmental distributions. 
\DIFaddbegin \DIFadd{An alternate visualization is also included in figure \ref{fig:qqplot_hexbins}.
}\DIFaddend For clinal environments, $-\log_{10}(p)$ values are most inflated when neighborhood sizes are large, consistent with the pattern observed in the count of significant associations after PC regression. In contrast corner and patchy environments cause the greatest inflation in $-\log_{10}(p)$ at neighborhood sizes less than 100, which likely reflects the inability of PCA to account for fine-scale structure caused by very limited dispersal. Finally, we observed that PC regression appears to overfit to some degree for all phenotype distributions, visible in Figure \ref{fig:gwas}D as points falling below the 1:1 line.

\begin{figure*}[p]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figures/gwas_summary_loglog.pdf}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figures/gwas_summary_loglog_qqlines.pdf}
    \DIFaddendFL \caption{Impacts of spatially varying environments and isolation by distance on linear regression GWAS. Simulated quantitative phenotypes are determined only by an individual's location and the spatial distribution of environmental factors. In \textbf{A} we show the phenotypes and locations of sampled individuals under four environmental distributions, with transparency scaled to phenotype. As neighborhood size increases a PCA explains less of the total variation in the data (\textbf{B}).
    ...
    Quantile-quantile plots in \DIFaddbeginFL \DIFaddFL{(}\DIFaddendFL \textbf{D}\DIFaddbeginFL \DIFaddFL{) }\DIFaddendFL show inflation of $-\log_{10}(p)$ after PC correction \DIFdelbeginFL \DIFdelFL{across all }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{for }\DIFaddendFL simulations \DIFdelbeginFL \DIFdelFL{and environmental distributions}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{with spatially structured environments}\DIFaddendFL , with \DIFaddbeginFL \DIFaddFL{line }\DIFaddendFL colors \DIFdelbeginFL \DIFdelFL{scaled by }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{showing }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{median }\DIFdelendFL neighborhood size \DIFdelbeginFL \DIFdelFL{in each region }\DIFdelendFL of \DIFdelbeginFL \DIFdelFL{q-q space}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{each simulation}\DIFaddendFL .
}
\label{fig:gwas}
\end{figure*}
...
When sampling is clustered at points on a landscape and dispersal is limited, the sampled individuals will be more closely related than a random set of individuals. Average coalescence times of individuals collected at a locality will then be more recent and branch lengths shorter than expected by analyses assuming random mating. This leads to fewer mutations and recombination events occurring since their last common ancestor, causing a random set of individuals to share longer average IBS tracts and have fewer nucleotide differences. For some data summaries, such as Tajima's $D$, Watterson's \DIFdelbegin \DIFdel{$\Theta$}\DIFdelend \DIFaddbegin \DIFadd{$\theta$}\DIFaddend , or the correlation coefficient between spatial distance and the count of long haplotype blocks, this can result in large differences in estimates between random and point sampling (Figure \ref{fig:sumstats}). 
Inferring underlying demographic parameters from these summary statistics 
-- unless the \DIFdelbegin \DIFdel{nature of the sampling is }\DIFdelend \DIFaddbegin \DIFadd{spatial locations of the sampled individuals are }\DIFaddend somehow taken into account 
-- will \DIFaddbegin \DIFadd{likely }\DIFaddend be subject to bias\DIFdelbegin \DIFdel{if sampling is not random across the landscape}\DIFdelend .
...
More surprisingly, midpoint sampling also leads to strongly positive Tajima's $D$ and an inflation in the proportion of heterozygous individuals in the sample -- similar to the effect of sampling a single deme in an island model as reported in \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Stadler2009}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{Stadler2009}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . 
...
We found that SMC++ was more robust to the effects of space than stairwayplot, underestimating recent populations by roughly half in the worst time periods rather than nearly 10-fold as with stairwayplot. Though this degree of variation in population size is certainly meaningful in an ecological context, it is relatively minor in population genetic terms. \DIFaddbegin \llabel{ibdnesxn}
\DIFadd{Methods directly assessing haplotype structure in phased data example, \mbox{%DIFAUXCMD
\citep[e.g.,][]{browning2015accurate} }\hspace{0pt}%DIFAUXCMD
are thought to provide increased resolution for recent demographic events,
...
Spatial structure is particularly challenging for genome-wide association studies, because the effects of dispersal on genetic variation are compounded by spatial variation in the environment \citep{Mathieson2012}. Spatially restricted mate choice and dispersal causes variation in allele frequencies across the range of a species. If environmental factors affecting the phenotype of interest also vary over space, then \DIFdelbegin \DIFdel{groups of individuals in different regions will }\DIFdelend allele frequencies and environmental exposures will covary over space. \DIFaddbegin \llabel{ll:1:11:2}
...
Other GWAS methods \DIFaddbegin \DIFadd{such as mixed models \mbox{%DIFAUXCMD
\citep{Kang2008} }\hspace{0pt}%DIFAUXCMD
}\llabel{mixedmodelcallout} \DIFaddend may be less sensitive to this confounding, but there is no obvious reason that this should be so. 
...
to get an idea of how \DIFdelbegin \DIFdel{likely dispersal is to play an important role in }\DIFdelend \DIFaddbegin \DIFadd{strongly local geographic dispersal affects }\DIFaddend patterns of variation.
\DIFdelbegin \DIFdel{Though this sample is almost certainly }\DIFdelend \DIFaddbegin \DIFadd{This is an imperfect measure:
some aspects of genetic variation are most strongly determined by neighborhood size \mbox{%DIFAUXCMD
\citep{Wright1946}}\hspace{0pt}%DIFAUXCMD
,
others (e.g., number of segregating sites)
are more strongly determined by global $N_e$ or by the ratio of the two.
%DIF >  These values should be compared to our simulation results with some caution, as though we expect neighborhood size to have similar effects across species of varying global $N_e$ \citep{Wright1946}, in our study we evaluated only a relatively small population of $\approx 10,000$. 
}\llabel{popsizecaveat}
\DIFadd{In addition, these empirical examples are likely }\DIFaddend biased towards small-neighborhood species (because few studies have quantified neighborhood size in species with very high dispersal or population density)\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{. However, from the available data }\DIFaddend we find that neighborhood sizes in the range we simulated are fairly common across a range of taxa. At the extreme low end of empirical neighborhood size estimates we see some flowering plants, large mammals, and colonial insects like ants. Species such as this have neighborhood size estimates small enough that spatial processes are likely to strongly influence inference.
These include some human populations such as the Gainj- and Kalam-speaking people of Papua New Guinea, in which the estimated neighborhood sizes in \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Rousset1997} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{Rousset1997} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend range from 40 to 410 depending on the method of estimation. \DIFaddbegin \revpoint{3}{7}
\DIFaddend Many more species occur in a middle range of neighborhood sizes between 100 and 1000 -- a range in which spatial processes play a minor role in our analyses under random spatial sampling but are important when sampling of individuals in space is clustered. Surprisingly, even some flying insects with huge census population sizes fall in this group, including fruit flies (\textit{D.\DIFaddbegin \DIFadd{~}\DIFaddend melanogaster}) and mosquitoes (\textit{A.\DIFaddbegin \DIFadd{~}\DIFaddend aegypti}). Last, many species likely have neighborhood sizes much larger than we simulated, including \DIFaddbegin \DIFadd{the recent ancestors of }\DIFaddend modern humans in northeastern Europe \citep{Ringbauer2017}. For these species demographic inference and summary statistics are likely to reflect minimal bias from spatial effects as long as dispersal is truly continuous across the landscape. While that is so we caution that association studies in which the effects of population structure are confounded with spatial variation in the environment are still sensitive to dispersal even at these large neighborhood sizes.


%%%%%%%%%%%
\DIFaddbegin \subsection{\DIFadd{Other demographic models}}

\DIFadd{Any simulation of a population of reproducing organisms
...
\DIFadd{The usual choice of population regulation for population genetics --
...
is sometimes referred to as the ``Bolker--Pacala model'' \mbox{%DIFAUXCMD
\citep{bolker1997using}}\hspace{0pt}%DIFAUXCMD
.
...
Since populations are still entirely }\emph{\DIFadd{intrinsically}} \DIFadd{regulated,
...
\DIFadd{Population genetic simulations often use grids of discrete demes, which are assumed to approximate continuous space. 
...
\mbox{%DIFAUXCMD
\citep{barton2002neutral}}\hspace{0pt}%DIFAUXCMD
.
...
%DIF > %%%%%%%%%%
\DIFaddend \subsection{Future Directions and Limitations}
...
One facet of spatial variation that we did not address in this study is the confounding of dispersal and population density implicit in the definition of Wright's neighborhood size. Our simulations were run under constant densities, but \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citet{Ringbauer2017}}\hspace{0pt}%DIFAUXCMD
's approach to demographic inference in space suggests that density and dispersal can in some cases be estimated separately from genetic data. }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{guindon2016demographic} }\hspace{0pt}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet{Ringbauer2017} }\hspace{0pt}%DIFAUXCMD
}\revpoint{3}{8}
...
\DIFaddend Much additional work remains to be done to better understand how these parameters interact to shape genetic variation in continuous space, which we leave to future studies. 
...
The reverse-time \DIFdelbegin \DIFdel{model of continuous space evolution }\DIFdelend \DIFaddbegin \DIFadd{spatial Lambda--Fleming--Viot model }\DIFaddend described by \citet{Barton2010} and implemented by \citet{Kelleher2014} 
allows exploration of \DIFdelbegin \DIFdel{parameter regimes with }\DIFdelend \DIFaddbegin \DIFadd{larger }\DIFaddend population and landscape sizes\DIFdelbegin \DIFdel{more directly comparable to empirical cases like humans}\DIFdelend \DIFaddbegin \DIFadd{,
but the precise connection of these models to forward-time demography is not yet clear}\DIFaddend .
...
Scripts used for all analyses and figures are available at \DIFdelbegin %DIFDELCMD < \url{https://github.com/petrelharp/spaceness}%%%
\DIFdelend \DIFaddbegin \url{https://github.com/kern-lab/spaceness}\DIFaddend . 

\section{Acknowledgements}
We thank Brandon Cooper, Matt Hahn, Doc Edge, and others for reading and thinking about this manuscript. 
\DIFdelbegin \DIFdel{We thank the Hearth for having such good, nearby coffee, and
Falling Sky Brewing for creative support during manuscript drafting.
CJB and ADK were supported by NIH award R01GM117241.
}\DIFdelend %DIF >  We thank the Hearth for having such good, nearby coffee, and
%DIF >  Falling Sky Brewing for creative support during manuscript drafting.
\DIFaddbegin \DIFadd{CJB and ADK were supported by NIH award R01GM117241. 
}\revpoint{AE}{4}
\DIFaddend 

\DIFdelbegin %DIFDELCMD < \bibliography{spaceness}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibliography{spaceness,more_refs}
\DIFaddend 
...
\setcounter{table}{0}
\renewcommand{\thetable}{A\arabic{table}}%DIF > 
\setcounter{figure}{0}
\renewcommand{\thefigure}{A\arabic{figure}}%DIF > 

%DIF > %%%%%%%%%%%%%%%%%%%%%%
\section{\DIFadd{Comparisons with Stepping-Stone Models}}
\llabel{apx:stepping_stone}

\DIFadd{We also compared our model results to a regular grid of discrete populations,
...
we implemented these using }\texttt{\DIFadd{msprime}} \DIFadd{\mbox{%DIFAUXCMD
\citep{Kelleher2016}}\hspace{0pt}%DIFAUXCMD
.
...
\centering
\begin{tabular}{ccc}
     \DIFaddFL{demes per side ($n$)}&\DIFaddFL{$N_e$ per deme  }& \DIFaddFL{samples per deme  }\\
     \hline
     \DIFaddFL{5 }& \DIFaddFL{244 }& \DIFaddFL{20 }\\
     \DIFaddFL{10 }& \DIFaddFL{61 }& \DIFaddFL{10 }\\
     \DIFaddFL{20 }& \DIFaddFL{15.25 }& \DIFaddFL{2 }\\
     \DIFaddFL{50 }& \DIFaddFL{2.44 }& \DIFaddFL{1 }\\
\end{tabular}
\captionsetup{justification=centering}
\caption{\DIFaddFL{stepping-stone simulation parameters}}
...
(equation }\eqref{eqn:p_survival}\DIFadd{):
}\begin{equation}
    \DIFadd{p = \min\left( 0.95, \frac{1}{1 + n / (K (1+L))} \right) .
}\end{equation}
\DIFadd{We capped survival at 0.95 so that we would not have exceptionally long-lived individuals
...
we get that in a well-mixed population, the equilibrium density should be around
    ...
    \DIFadd{n = K \frac{L+1}{L-1}
}\end{equation}
\DIFadd{individuals per unit area.
At this density, the per-capita death rate is $1/L$, so the mean lifetime is $L$.
This equilibrium density is }\emph{\DIFadd{not}} \DIFadd{$K$, but (since $L=4$) is two-thirds larger.
...
by dividing census size by geographic area \mbox{%DIFAUXCMD
\citep{lloyd1967crowding}}\hspace{0pt}%DIFAUXCMD
.
...
\DIFaddend \section{Supplementary Figures and Tables}
...
\centering
\DIFaddbeginFL \includegraphics[width=\textwidth]{figures/wm_v_dd_maps.pdf}
\caption{\DIFaddFL{Maps of individual locations in a continuous-space Wright-Malécot model with independent dispersal of all individuals (top) and under our continuous space model incorporating density-dependant fitness (bottom). The clustering seen in the top row is the ``Pain in the Torus'' described by \mbox{%DIFAUXCMD
\citet{Felsenstein1975}}\hspace{0pt}%DIFAUXCMD
.}}
\label{fig:wm_v_dd} 
\end{figure*}
\FloatBarrier
...
\caption{\DIFaddFL{Comparison of individual fitness across the landscape in simulations with (right) and without (left) a decline in fitness approaching range edges. Note the slight excess of high-fitness individuals at edges on the left, which is (partially) counteracted by the scaling procedure.}}
...
\caption{\DIFaddFL{Site frequency spectra from a simulation with neighborhood size = 12.5 when mutations are recorded directly in SLiM (blue line) or applied later in msprime (red line).}}
...
\begin{figure*}[p]
\centering
\DIFaddendFL \includegraphics[width=\textwidth]{figures/sumstats_by_neighbors_allstats.pdf}
...
\caption{\DIFdelbeginFL \DIFdelFL{Normalized mean }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Variation in }\DIFaddendFL observed heterozygosity \DIFdelbeginFL \DIFdelFL{by location }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(i.e. proportion of heterozygous individuals) in hexagonal bins }\DIFaddendFL across \DIFaddbeginFL \DIFaddFL{the landscape, estimated from a random sample of }\DIFaddendFL 200 \DIFdelbeginFL \DIFdelFL{randomly sampled }\DIFdelendFL individuals \DIFaddbeginFL \DIFaddFL{from the final generation of a simulation with neighborhood size $\approx$25. Values were Z-normalized for plotting.}\DIFaddendFL }
\label{fig:hetmap}
\end{figure*}
\DIFaddbegin \FloatBarrier
\DIFaddend 
...
\DIFdelbegin %DIFDELCMD < \stopsupplement
%DIFDELCMD < %%%
