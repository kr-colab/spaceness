...
defined to be $N_W = 4 \pi \rho \sigma^2$, where $\sigma$ is the mean parent--offspring distance along each of the x and y axes and $\rho$ is population density (see \citep{Rousset1997} for further discussion of parameter defiitions in one- and two-dimensional habitats) \llabel{axialdist}. 
...
As in previous work \citep{Wright1943,Ringbauer2017}, $\sigma$ is equal to the mean parent-offspring distance along each of the x and y axes.  
...
Essentially, PCA seems unable to effectively summarize the weak population structure present in large-neighborhood simulations given the sample sizes we tested, \revpoint{2}{2} \llabel{samplesizenote}
...
are more strongly determined by global $N_e$ or by the ratio of the two, and definitions of "population density" in genetic versus ecological studies may lead to varying estimates of neighborhood size for a given species \llabel{rhocaveat}. 
...
Indeed, our finding that stepping-stone models reproduce only some of the effects on genetic variation seen in a continuous-space model (Figure \ref{fig:grid_stats}) suggests that qualitatively similar models can produce different results dependent on the specific parameterizations used. While we believe our continuous model is a more appropriate depiction of evolution in a continuous landscape than lattice models, it is likely that some populations and breeding systems do more closely resemble a series of interconnected random-mating populations. As with all population models, the best approximation for any empirical system will depend on the natural history of the species in question. \llabel{bestmodelcaveat}
Much additional work remains to be done to better understand how life history and range size interact to shape genetic variation in continuous space, which we leave to future studies. 
